...
import Logo from "../assets/icons/Salon.png";
  ...
  useEffect(() => {
    ...
    return (
      <div>
            ...
            {navLinks.map((link) => (
              <li key={link.name} className="relative group">
                ...
                >
                  {({ isActive }) => (
                    <>
                      <span className="relative z-10 bg-gradient-to-r from-[#f042ff] to-[#f775b1] bg-clip-text text-transparent">
                        {link.name}
                      </span>
                      <span
                        className={`absolute left-0 bottom-0 w-full h-0.5 bg-blue-300 transition-transform duration-300 origin-left pointer-events-none ${
                          isActive
                            ? "scale-x-100"
        ...
        {/* Mobile Menu */}
        {menuOpen && (
          <div ref={menuRef} className="md:hidden rounded-bl-2xl rounded-br-2xl sticky mt-[-20px] left-0 w-full bg-white/60 p-4 text-xs font-Suwannaphum backdrop-blur-md z-40 shadow-lg animate-fade-in">
              ...
              {navLinks.map((link) => (
                <li key={link.name}>
                  <NavLink
                    ...
                    className={({ isActive }) =>
                      `block px-1 pb-1 transition-colors duration-200 ${
                        isActive ? "bg-gradient-to-r from-[#da7ee3] to-[#f775b1] bg-clip-text text-transparent" : "hover:text-blue-300"
                      }`
                    }
